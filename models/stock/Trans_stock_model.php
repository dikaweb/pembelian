...
class trans_stock_model extends CI_Model
{
    public function save_m()
    {
        ...
        $query1 = $this->db->query("select no_urut as max_id from transfer_stock where no_urut = (SELECT MAX(no_urut) FROM transfer_stock)");
        $row = $query1->row_array();
        if (isset($row['max_id'])) {
            $no_urut = $row['max_id'] + 1;

            $mulai = $row['max_id'] - 1;
            $query2 = $this->db->query("select no_urut from transfer_stock where no_urut > $mulai order by no_urut");
        ...
        $no_po = "TRANSFER-"  . sprintf("%04s", $no_urut);
        $data = [
            //'no_transaksi' => $post['no_transaksi'],
            'no_transfer_stock' => $no_po,
            'no_urut' => $no_urut,
            'tanggal' =>  date("Y-m-d"),
        ...
        $row = $this->db->query('select max(id_transfer_stock) as id_transaksi from transfer_stock ')->row_array();
        ...
        if ($akhir >= 0) {
            $data2 = [
                'id_transfer_stock' => $id_transaksi,
                'id_barang' => $id_barang,
            ...
            $this->db->insert('transfer_stock_d', $data2);
        } else {
            $dika = '<div class="alert alert-danger" role="alert">Jumlah ' . $jumlah . ', Nilai Konversi terkecil: ' . $hasil .  ', Jml Stock : ' . $row['qty'] . '. <br> Gagal disimpan, Jumlah melebihi dari Stock <br> Ulangi Input Barang !!!</div>';
    ...
    {
        $query = "SELECT * from transfer_stock a
        where a.id_transfer_stock = $id_transaksi
        ";
    ...
    {
        $query = "SELECT * from transfer_stock_d a
        ...
        where a.id_transfer_stock = $id_transaksi order by id_transfer_stock_d
        ";
    ...
    public function update_m()
    {
        ...
        $this->db->update('transfer_stock', $data, array('id_transfer_stock' =>  $post["txtid_transaksi"]));
    }
}
