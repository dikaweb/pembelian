...
class peng_stock_model extends CI_Model
{
    public function save_m()
    {
        ...
        $query1 = $this->db->query("select no_urut as max_id from trans_pengeluaran where no_urut = (SELECT MAX(no_urut) FROM trans_pengeluaran)");
        $row = $query1->row_array();
        if (isset($row['max_id'])) {
            $no_urut = $row['max_id'] + 1;

            $mulai = $row['max_id'] - 1;
            $query2 = $this->db->query("select no_urut from trans_pengeluaran where no_urut > $mulai order by no_urut");
        ...
        $no_po = "PENGELUARAN-"  . sprintf("%04s", $no_urut);
        $data = [
            //'no_transaksi' => $post['no_transaksi'],
            'no_pengeluaran' => $no_po,
            'no_urut' => $no_urut,
            'tanggal' =>  date("Y-m-d"),
        ...
        $this->db->insert('trans_pengeluaran', $data);


        $row = $this->db->query('select max(id_pengeluaran) as id_transaksi from trans_pengeluaran ')->row_array();
        ...
        if ($akhir >= 0) {
            ...
            $this->db->insert('trans_pengeluaran_d', $data2);
        } else {
    ...
    public function konfirmasi_m($id_transaksi)
    {
        ...
        where a.id_pengeluaran = $id_transaksi
        ";
    ...
    {
        $query = "SELECT * from trans_pengeluaran_d a
        ...
        where a.id_pengeluaran = $id_transaksi order by id_pengeluaran_d
        ";
