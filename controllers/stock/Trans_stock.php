...
class trans_stock extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        is_logged_in();
        $this->load->model("stock/trans_stock_model");
    }
    ...
    public function index()
    {
        ...
        $url = 'stock/trans_stock/view/0/' . $dateawal . '/' . $datenow;
        redirect($url);
    }
    ...
    public function view($id_company, $startDate, $endDate)
    {
        ...
        $query = " select *,nama_lengkap as name from transfer_stock a 
        ...
        $this->load->view('stock/v_trans_stock', $data);
    ...
    public function add()
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('stock/v_trans_stock_add', $data);
    }
    ...
    public function edit($id_transaksi)
    {
        ...
        $data['konfirmasi_m'] = $this->trans_stock_model->konfirmasi_m($id_transaksi);
        $data['konfirmasi_d'] = $this->trans_stock_model->konfirmasi_d($id_transaksi);

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('stock/v_trans_stock_edit', $data);
    }

    public function save_m()
    {
        echo json_encode($this->trans_stock_model->save_m());
    }

    public function save_d()
    {
        echo json_encode($this->trans_stock_model->save_d());
    }
    ...
    public function delete_m()
    {
        ...
        $this->db->where('id_transfer_stock', $post['idd']);
        $this->db->delete('transfer_stock_d');
        $this->db->where('id_transfer_stock', $post['idd']);
        $this->db->delete('transfer_stock');
        $this->session->set_flashdata('messagez', $dika);
        redirect('stock/trans_stock');
    }

    public function delete_d()
    {
        $post = $this->input->post();
        $this->db->where('id_transfer_stock_d', $post['idd']);
        $this->db->delete('transfer_stock_d');
        $id_transaksi =  $post['idt'];
        $url = 'stock/trans_stock/edit/' . $id_transaksi;
        redirect($url);
    }
    ...
    {
        $this->trans_stock_model->update_m();
    ...
    public function pilihbarang($id_gudang)
    {
        ...
        where a.id_gudang  = $id_gudang and qty <> 0";
        $data['barang'] = $this->db->query($query)->result_array();
        $this->load->view('stock/v_trans_stock_modal_barang', $data);
    }
    public function pilihgudang($jenis)
    {
        ...
        $data['jenis'] = $jenis;
        $this->load->view('stock/v_trans_stock_modal_gudang', $data);
    }
}
