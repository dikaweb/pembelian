...
class daftar_surat extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $url = 'surat/daftar_surat/view/' . $dateawal . '/' . $datenow;
        redirect($url);
    }
    ...
    public function view($startDate, $endDate)
    {
        ...
        left join m_kode_jenis_dept g on a.id_kode_jenis_dept = g.id_kode_jenis_dept
        where a.tgl_upload  between '$startDate' and '$endDate' 
        order by a.no_urut
        ";
        }
        ...
        left join status f on a.status = f.id_status
        left join m_kode_jenis_dept g on a.id_kode_jenis_dept = g.id_kode_jenis_dept
        where (a.id_dept = $id_dept or a.id_user = $id_user) and a.tgl_upload  between '$startDate' and '$endDate' 
        order by a.no_urut
        ";
        }
        ...
        if ($v_all == 0 && $v_all_dept == 0) {
            ...
            left join status f on a.status = f.id_status
            left join m_kode_jenis_dept g on a.id_kode_jenis_dept = g.id_kode_jenis_dept
            where a.id_user = $id_user and a.tgl_upload  between '$startDate' and '$endDate' 
            order by a.no_urut";
        }
        ...
        $this->load->view('surat/v_daftar_surat', $data);
    }
