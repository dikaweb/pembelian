                ...
                <table class="table table-striped table-bordered" id="konfirmasiTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php foreach ($konfirmasi as $mr) : ?>
                            <tr>
                                <td width="15%">
                                    <a href="<?= base_url('stock/peng_stock/edit/') . $mr['id_pengeluaran']; ?>" class="badge badge-success clastomboledit">edit</a>
                                    <?php if ($mr['id_user'] == $user['id']) { ?>
                                        <a href="#" class="badge badge-danger clastomboldel" data-status="<?= $mr['status']; ?>" data-user="<?= $mr['nama_lengkap']; ?>" data-id="<?= $mr['id_pengeluaran']; ?>" data-nomor="<?= $mr['no_pengeluaran']; ?>" data-tanggal="<?= tgl_indo($mr['tanggal']); ?>" data-toggle="modal" data-target="#deleteMenuModal">delete</a>
                                    <?php } ?>
                        ...
                        -->
                                ...
                                <td><?= $mr['no_pengeluaran']; ?></td>
                                <td><?= tgl_indo($mr['tanggal']); ?></td>
                                <td>
                                    <?php
                                    $menuId = $mr['id_pengeluaran'];
                                    $querySubMenu = "SELECT * from trans_pengeluaran_d a 
...
<div class="modal fade" id="deleteMenuModal" tabindex="-1" role="dialog" aria-labelledby="deleteMenuModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('stock/peng_stock/delete_m'); ?>" method="post">
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#konfirmasiTable').DataTable({
            ...
            'fnDrawCallback': function(oSettings) {
                $('.dataTables_filter').each(function() {
                    $('.tomboltambah').remove();
                    $(this).append('<a class="btn btn-primary mb-2 mt-2 mx-2 btn-sm tomboltambah" id="tomboltambah" href="<?= base_url('stock/peng_stock/add'); ?>">Tambah Transaksi</a>');
                });
            },
            fixedColumns: {
                heightMatch: 'none'
        ...
        $('#daterange').on('apply.daterangepicker', function(ev, picker) {
            ...
            url = "<?= base_url('stock/peng_stock/view/'); ?>" + id_company + '/' + $t1 + '/' + $t2;
