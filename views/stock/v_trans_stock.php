                ...
                <table class="table table-striped table-bordered" id="konfirmasiTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php foreach ($konfirmasi as $mr) : ?>
                            <tr>
                                <td width="15%">
                                    <a href="<?= base_url('stock/trans_stock/edit/') . $mr['id_transfer_stock']; ?>" class="badge badge-success clastomboledit">edit</a>
                                    <?php if ($mr['id_user'] == $user['id']) { ?>
                                        <a href="#" class="badge badge-danger clastomboldel" data-status="<?= $mr['status']; ?>" data-user="<?= $mr['name']; ?>" data-id="<?= $mr['id_transfer_stock']; ?>" data-nomor="<?= $mr['no_transfer_stock']; ?>" data-tanggal="<?= tgl_indo($mr['tanggal']); ?>" data-toggle="modal" data-target="#deleteMenuModal">delete</a>
                                    <?php } ?>
                                    <!--
                                    <a href="#" class="badge bg-gradient-warning text-gray-100 clastombolview" data-id="<?= $mr['id_transaksi']; ?>">view</a>
                        -->
                                </td>
                                <td><?= $mr['no_transfer_stock']; ?></td>
                                <td><?= tgl_indo($mr['tanggal']); ?></td>
                                <td>
                                    <?php
                                    $menuId = $mr['id_transfer_stock'];
                                    $querySubMenu = "SELECT * from transfer_stock_d a 
                                    ...
                                    where a.id_transfer_stock = $menuId";
                                    $subMenu = $this->db->query($querySubMenu)->result_array();
                                    $x = 1;
                                    foreach ($subMenu as $m2) :
                                        echo $x . ") " . $m2['jumlah'] . " "  . $m2['nm_satuan'] . " "  . $m2['nm_barang'] . " dari "  . $m2['kd_gudang'] . "<br>";
                                        $x++;
                                    endforeach;
                                    ?>
                                </td>
...
<div class="modal fade" id="deleteMenuModal" tabindex="-1" role="dialog" aria-labelledby="deleteMenuModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('stock/trans_stock/delete_m'); ?>" method="post">
        ...
        $('#konfirmasiTable').DataTable({
            ...
            'fnDrawCallback': function(oSettings) {
                $('.dataTables_filter').each(function() {
                    $('.tomboltambah').remove();
                    $(this).append('<a class="btn btn-primary mb-2 mt-2 mx-2 btn-sm tomboltambah" id="tomboltambah" href="<?= base_url('stock/trans_stock/add'); ?>">Tambah Transaksi</a>');
                });
            },
            fixedColumns: {
                heightMatch: 'none'
