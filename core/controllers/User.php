...
class User extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $id = $this->session->userdata('email2');
        $query = "SELECT a.email as email, a.name as name, b.role as role, a.id as id,b.id as id_role
        ,c.lokasi as lokasi,d.nm_dept as departement
        FROM user a 
        JOIN user_role b ON a.role_id = b.id 
        left JOIN lokasi c ON a.id_lokasi = c.id 
        left JOIN m_dept d ON a.id_dept = d.id_dept 
        where email = '$id'
      ";
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!password_verify($current_password, $data['user']['password'])) {
                ...
                if ($current_password == $new_password) {
                    ...
                    $this->db->where('email', $this->session->userdata('email2'));
                    $this->db->update('user');
